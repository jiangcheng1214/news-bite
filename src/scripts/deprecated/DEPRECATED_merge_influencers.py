...
def is_porn_influencer(api_manager: TwitterAPIManager, influencer_data_json):
    ...
    for porn_keyword in porn_keywords:
        if description.lower().find(porn_keyword) >= 0:
            matched += 1
        if matched >= 1:
            return True
    ...
    for tweet in recent_posted_tweets:
        for porn_keyword in porn_keywords:
            if tweet.lower().find(porn_keyword) >= 0:
                total_matched += 1
                continue
...
def group_influencers(source_dir_paths, output_csv_dir, tell_porn=False):
    """
    ...
    influencers_output_csv_path = os.path.join(
        output_csv_dir, 'influencers.csv' if tell_porn else 'all_influencers.csv')
        ...
        for file_name in sorted(os.listdir(source_dir_path)):
            if file_name.startswith(file_prefix):
                for line in open(os.path.join(source_dir_path, file_name)):
                    ...
                    if tell_porn:
                        try:
                            is_porn = is_porn_influencer(
                                api_manager, influencer_data_json)
                        except:
                            print("is_porn_influencer check failed for " + username)
                            is_porn = False
                        print(f"{username} - is_porn: {is_porn}")
                        if is_porn:
                            porn_influencers_csv_file.write(
